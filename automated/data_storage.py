...
"""
Data handling for the active learning and 
logging methods
"""
class wafer():
    """
...
if __name__ == "__main__":
    ...
    s = spec()
    lsa = LSA()
    data = s.read_spec("../pdc-max/Analyze_Bi2O3_19F44/Bi2O3_19F44/Spectrometer/Data/s+02_+35_00900_0960.csv")
    s.write_csv_spec("TMP.csv", data["Meta"], data["Wavelengths"], data["Spec"])
    print(data)
    mirror = s.read_mirror_blank("../pdc-max/Analyze_Bi2O3_19F44/Bi2O3_19F44/Spectrometer/Data/mirror_30.csv")
    blank = s.read_mirror_blank("../pdc-max/Analyze_Bi2O3_19F44/Bi2O3_19F44/Spectrometer/Data/blank_20.csv")
    #plt.plot(data["Spec"][:, 10], label = "Data")
    #plt.plot(mirror["Spec"], label = "Mirror")
    #plt.plot(blank["Spec"], label = "Blank")
    normal_data = s.normalize(data["Spec"], mirror["Spec"], blank["Spec"])
    spects = s.user_coord(data)
    #plt.legend()
    #plt.show()

    for sp in spects:
        start = time.time()
        ss = s.normalize(sp["Spec"], mirror["Spec"], blank["Spec"])
        wl = sp["Wavelengths"]
        end = time.time()
        plt.plot(wl, ss)
        print(end - start)
    ...
    plt.imshow(data["Spec"], aspect = 'auto')
    plt.show()
    plt.imshow(normal_data[400:1600, :], aspect = 'auto')
    plt.show()

    lsa_map = lsa.read("/home/maxamsler/Downloads/18CIT49586_LaMnOx.csv")
    print(lsa_map)
