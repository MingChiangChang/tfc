...
class collection(object):
    ...
    def set_camera_ring_size(self):
        ...
        recv = self.clients["camera"].set_ZOOCAM_RING_SET_SIZE(self.msg_id,
                                                               args.ringsize)
        if recv < 0:
            print("Could not set ring size")
            sys.exit(10) # Anything other than 0 is consider abnormal exit
        else:
    ...
    def arm_camera(self):
        ...
        recv = self.clients["camera"].set_ZOOCAM_SET_TRIGGER_MODE(
                self.msg_id,
                trigger_mode=2,
    ...
    def run_lsa(self, power = None):
        ...
        recv["Units"] = 1  # TODO: Check if these are the right laser and unit 
        recv["Laser"] = 0  # 
        recs = [recv]

        #Set job parameters
        jobstr = self.clients["lasgo"].get_LASGO_GET_JOB_STRUCT(self.msg_id, 1)
        jobstr["MaxAccel"] = 5.
        jobstr["UseRobustPower"] = True
        jobstr_recv = self.clients["lasgo"].set_LASGO_SET_JOB_STRUCT(self.msg_id, jobstr)
        print("Job setting", jobstr)

        #Set zone
        rec = self.clients["lasgo"].set_LASGO_VALIDATE_ZONE_SCAN(
                self.msg_id,
                option,
                recs[0]
            ) #Should rewrite handling zones (plural)
        if rec !=0:
            print(("Lasgo returns that the requested zone cannot be executed"
                   "Returning with the return code"))
            print("Error code", rec)
            #sys.exit()

        #Execute scan
        recv = self.clients["lasgo"].set_LASGO_EXECUTE_ZONE_SCAN(
                self.msg_id,
                option,
                recs,
                timeout = 100000
    ...
    def save_images(self, file_format, path):
        ...
        Options:
            ...
            png: "png" or 4
        """
        image_info = self.clients['camera'].set_ZOOCAM_SAVE_FRAME(
                self.msg_id,
                1,
                file_format,
                path)
    ...
    def get_headers_and_images(self):
        ...
        for i_image in range(n_images):
            ...
            recv = self.clients['camera'].get_ZOOCAM_GET_IMAGE_DATA(
                    self.msg_id,
                    image_info,
                    frame_id=i_image,
                    get_raw = False,
    ...
    def run(self, power = None, save_local = False,
        ...
        if self.args.savelocal:
            ...
            print("###############")
            print("Image shape", np.array(self.images).shape)
            print("###################")
            #Write files
            if power is None:
                self.write_file()
